...
- We don't test against Prod because it is too risky to lose client's data. It may be more or less safe to test Collection Types because we add a new entry and it doesn't affect the rest of the entries. But this is not the case with Single Types. So is this potentailly OK to test Collection but not Single types?
...
In case you need to update the component, you need to regenerate its screenshot so it was up-to-date with the component's new appearance.

#### What we test
Components with static data.</br>
If a component has several states, we can test them with screenshots, e.g. an accordion in both collapsed and open state.</br>
...
### Unit Testing
ToDo
- How do we test the work of functions e.g. filtration? We can test the request with unit tests on Jest, but seems like we are missing some kind of component testing? To make sure that this request is triggered on click.
...
#### What we test
We can check such functionality as e.g. calculations or requests to CMS (Preview mode, Filtration, Sorting). 
...
- labels
- etc. </br>
These checks are based on [WCAG principles](https://www.w3.org/WAI/standards-guidelines/wcag/), AA level.
